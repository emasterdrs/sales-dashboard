...
// Also need to regenerate ALL_PRODUCTS if it exists.
let apStr = 'export const ALL_PRODUCTS = Object.values(PRODUCT_TYPES).flat().map(p => ({\n    ...p,\n    type: Object.keys(PRODUCT_TYPES).find(k => PRODUCT_TYPES[k].some(item => item.code === p.code))\n}));\n';
s = s.replace(/export const ALL_PRODUCTS = \[.*?\];/s, apStr);
s = s.replace(/export const ALL_PRODUCTS = Object\.values\(PRODUCT_TYPES\).*?\}\)\);\n/s, apStr);
